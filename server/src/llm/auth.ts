...
export type AuthConfig =
  ...
  | { mode: "oauth"; credentials: ClaudeCredentials }
  | { mode: "bedrock"; region: string };

const VALID_MODES: ReadonlySet<AuthMode> = new Set(["api", "oauth", "bedrock"]);

function readAuthMode(): AuthMode {
  const raw = process.env.CLAUDE_AUTH?.trim().toLowerCase();
  if (!raw) {
    throw new Error(
      "CLAUDE_AUTH is not set. Choose one of: api, oauth, bedrock.\n" +
  ...
  if (!VALID_MODES.has(raw as AuthMode)) {
    throw new Error(
      `CLAUDE_AUTH="${raw}" is invalid. Must be one of: api, oauth, bedrock.`
    );
  }
...
export function resolveAuth(): AuthConfig {
  ...
  if (mode === "api") {
    ...
    if (!apiKey) {
      throw new Error(
        "CLAUDE_AUTH=api requires ANTHROPIC_API_KEY to be set."
      );
    }
    return { mode, apiKey };
  }
  ...
  if (mode === "oauth") {
    const credentials =
      getClaudeCredentials() || getClaudeKeychainCredentials();
    if (!credentials) {
      throw new Error(
        "CLAUDE_AUTH=oauth requires Claude Code credentials. Run: claude login"
      );
    }
  ...
  if (!region) {
    throw new Error(
      "CLAUDE_AUTH=bedrock requires AWS_REGION to be set " +
