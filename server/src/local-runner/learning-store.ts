...
interface LearningState {
  runs: Array<{ at: string; runDir: string; status: string; task: string; url?: string; analysis?: any }>;
  mistakes: Array<{ at: string; action: string; count: number; sample: string }>;
}

export class LearningStore {
  private state: LearningState = { runs: [], mistakes: [] };
  ...
  async load(): Promise<void> {
    try {
      this.state = JSON.parse(await readFile(this.filePath, "utf8")) as LearningState;
...
${mistakes || "(none)"}
  ...
  async recordRun(params: { runDir: string; status: string; task: string; url?: string; turns: any[]; analysis?: any }): Promise<void> {
    ...
    const next = this.writeChain.then(async () => {
      ...
      for (const turn of params.turns || []) {
        for (const tool of turn.tools || []) {
          ...
          if (!/error|failed|stale|missing|unsupported/i.test(result)) continue;
