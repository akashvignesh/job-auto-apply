...
async function loadJobs(jobsPath: string): Promise<Job[]> {
  ...
  for (const line of raw.split(/\r?\n/)) {
    ...
    try {
      const j = JSON.parse(trimmed) as Job;
      if (!j.task) {
...
async function runOneTask(
  ...
  try {
    ...
    if (!target.webSocketDebuggerUrl) throw new Error(`Chrome target on port ${debugPort} has no debugger URL`);
...
async function runPool(opts: {
  ...
  const workers = Array.from({ length: opts.concurrency }, (_, workerIdx) =>
    (async () => {
      while (true) {
        const myIdx = nextJobIdx++;
        if (myIdx >= opts.jobs.length) return;
        const job = opts.jobs[myIdx];
        const r = await runOneTask(
          job,
          workerIdx,
          opts.basePort + workerIdx,
          opts.defaultMaxSteps,
          opts.runsDir,
          opts.profile,
          opts.patternStore,
          opts.learning,
...
Outputs:
  ...
  if ((byStatus["complete"] || 0) < jobs.length) process.exitCode = 2;
}
