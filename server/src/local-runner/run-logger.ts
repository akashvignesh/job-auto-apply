...
export class RunLogger {
  ...
  async saveScreenshot(base64: string): Promise<void> {
    this.screenshotCount++;
    await writeFile(join(this.dir, `screenshot_${this.screenshotCount}.jpg`), Buffer.from(base64, "base64"));
  }

  async complete(params: { status: string; model?: string; usage: LocalRunLog["usage"]; turns: any[]; analysis?: any }): Promise<void> {
  ...
  private async writeReplayArtifacts(turns: any[]): Promise<void> {
...
export function analyzeTurns(turns: any[]): any {
  ...
  const readPageByMode: Record<string, number> = {
  ...
  for (const turn of turns || []) {
    if (turn.patternReplay) {
      ...
      const hadCompleteMarker = (turn.tools || []).some((t: any) =>
        typeof t.result === "string" && /Page fill complete via pattern replay/.test(t.result)
      );
      if (hadCompleteMarker) patternReplaySuccess += 1;
    }
    ...
    for (const tool of turn.tools || []) {
      ...
      const isScreenshotCall =
        (tool.name === "computer" && tool.input?.action === "screenshot")
      ...
      if (/^Error: BLOCKED:.*identical/.test(resultText)) blockedRepeatedFailure += 1;
      if (/^Error: BLOCKED: read_page mode="full"/.test(resultText)) blockedFullReadThrottle += 1;
      if (/^Error: computer call|^Error: computer action/.test(resultText)) blockedMalformedComputer += 1;
      ...
      if (/BLOCKED \(policy\):.*FINAL submit/.test(resultText)) blockedSubmitClick += 1;
      if (/BLOCKED \(policy\): cross-domain navigation/.test(resultText)) blockedCrossDomainNav += 1;

      // Playwright actionability — pre-action gates added after the URL audit.
      if (/element is DISABLED/.test(resultText)) {
        if (tool.name === "computer") blockedDisabledClick += 1;
        else if (tool.name === "form_input") blockedDisabledFormInput += 1;
      }
      if (/element is READONLY/.test(resultText)) blockedReadonlyFormInput += 1;

      // Phase 7.4 date adapter / verification.
      if (tool.name === "form_input" && /\[reformatted .* to match /.test(resultText)) dateAdaptedFills += 1;
      if (tool.name === "form_input" && /the field is empty after blur/.test(resultText)) formInputRejectedRevert += 1;

      // Stale-ref self-heal logs from element-resolver.
      if (/self-heal succeeded|stale ref recovered/i.test(resultText)) staleRefSelfHeals += 1;

      // Page-complete signal from pattern replay.
      if (/Page fill complete via pattern replay/.test(resultText)) pagesComplete += 1;

      if (/error|failed|stale|missing|BLOCKED/i.test(resultText)) failures++;
 ...
 */
export function formatMetricsSummary(usage: { inputTokens: number; outputTokens: number; apiCalls: number }, analysis: any): string {
  const a = analysis || {};
...
function extractReplayableActions(turns: any[]): Array<{ name: string; input: any; turn?: number }> {
  const replayable = new Set(["navigate", "computer", "form_input", "run_script", "javascript_tool", "verify_action"]);
  const actions: Array<{ name: string; input: any; turn?: number }> = [];
  for (const turn of turns || []) {
    for (const tool of turn.tools || []) {
      if (!replayable.has(tool.name)) continue;
