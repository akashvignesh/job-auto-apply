...
export class LocalBrowser {
  constructor(private cdp: CdpSession, private screenshotSink?: (base64: string) => Promise<void> | void) {}
  ...
  private async navigate(url: string): Promise<ToolResult> {
    if (url === "back") {
      ...
      if (!previous) return { success: false, error: "No browser history entry to navigate back to" };
      await this.cdp.send("Page.navigateToHistoryEntry", { entryId: previous.id });
      await this.wait(800);
      return { success: true, output: "Navigated back" };
    }
    if (!/^https?:\/\//i.test(url)) return { success: false, error: `Invalid URL: ${url}` };
    await this.cdp.send("Page.navigate", { url });
    await this.wait(1200);
    return { success: true, output: `Navigated to ${url}` };
  }

  private async readPage(maxChars: number): Promise<ToolResult> {
    const result = await this.evaluate(READ_PAGE_SCRIPT);
    if (result.error) return result;
    const data = result.output as { url: string; title: string; bodyText: string; elements: string[] };
  ...
  private async getPageText(maxChars: number): Promise<ToolResult> {
  ...
  private async formInput(ref: string, value: any): Promise<ToolResult> {
  ...
  private async computer(input: Record<string, any>): Promise<ToolResult> {
  ...
  private async runScript(input: Record<string, any>): Promise<ToolResult> {
    ...
    for (let i = 0; i < actions.length; i++) {
      ...
      if (!res.success && input.stopOnError) break;
    }
    return { success: fail === 0, output: `run_script: ${ok} OK, ${fail} FAIL\n${lines.join("\n")}` };
  }

  private async verifyAction(input: Record<string, any>): Promise<ToolResult> {
    ...
    while (Date.now() - started < timeoutMs) {
      if (expect === "text-appeared") {
        ...
        if (String(r.output || "").includes(hint)) return { success: true, output: `verify_action OK: text appeared ${hint}` };
      } else if (expect === "navigated") {
        const r = await this.evaluate("location.href");
        if (!hint || String(r.output || "").includes(hint)) return { success: true, output: `verify_action OK: ${r.output}` };
      }
      await this.wait(250);
    }
  ...
  private async javascriptTool(input: Record<string, any>): Promise<ToolResult> {
    if (input.action && input.action !== "javascript_exec") return { success: false, error: "'javascript_exec' is the only supported action" };
  ...
  private async screenshot(): Promise<ToolResult> {
    const res = await this.cdp.send<{ data: string }>("Page.captureScreenshot", { format: "jpeg", quality: 75, fromSurface: true });
  ...
  private async evaluate(expression: string): Promise<ToolResult> {
    try {
      const res = await this.cdp.send<any>("Runtime.evaluate", {
        expression,
        awaitPromise: true,
        returnByValue: true,
      });
      const value = res.result?.value;
      if (value && typeof value === "object" && value.error) return { success: false, error: String(value.error) };
      if (value && typeof value === "object" && value.output) return { success: true, output: value.output };
