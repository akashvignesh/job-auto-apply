...
function buildRequestBody(system, messages, tools, maxTokens) {
...
function convertMessages(messages) {
    ...
    for (const msg of messages) {
            ...
            if (block.type === "text") {
                ...
                if (typeof tr.content === "string") {
                    ...
                    for (const c of tr.content) {
                        if (c.type === "text")
                            parts.push({ text: c.text || "(empty)" });
                        else if (c.type === "image")
                            parts.push({ text: IMAGE_PLACEHOLDER });
                    }
                }
                if (parts.length === 0)
        ...
        if (textChunks.length > 0)
            content.push({ text: textChunks.join("\n") });
...
// ─── Main call ────────────────────────────────────────────────────────────────
export async function callBedrockLLM(params) {
    const apiKey = process.env.BEDROCK_API_KEY;
    if (!apiKey)
        throw new Error("BEDROCK_API_KEY not set.");
    const { messages, system, tools, model = process.env.BEDROCK_MODEL || DEFAULT_MODEL, maxTokens = DEFAULT_MAX_TOKENS, signal, onText, } = params;
