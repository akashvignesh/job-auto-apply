...
import { parseRefsFromDom, extractFingerprintFromDom, } from "../local-runner/page-pattern-store.js";
 ...
 */
async function attemptPatternReplay(pattern, domText, profile, patternStore, executeTool, onStep, step) {
    try {
        ...
        console.error(`[PatternReplay] ${pattern.formLabel || pattern.fingerprint}: replaying ${actions.length}/${pattern.steps.length} steps`);
...
export async function runAgentLoop(params) {
    const { task, url, context, executeTool, onStep, onText, maxSteps = 50, signal, patternStore, profile = {}, previouslyCompleted = [], onPageComplete, model, } = params;
    ...
    let consecutiveFullReadDomKey = null;
                    ...
                    if (completedPages.has(fingerprint)) {
                        ...
                        if (domain) {
                            ...
                            if (pattern && pattern.successCount >= 2) {
                                ...
                                const replayResult = await attemptPatternReplay(pattern, domText, flatProfile, patternStore, executeTool, onStep, step);
