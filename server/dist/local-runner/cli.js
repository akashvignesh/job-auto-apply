 ...
 */
async function exportPatternsFromResult(result, patternStore, targetUrl) {
    if (!result.completedPages || result.completedPages.length === 0)
        return;
    let domain = "";
    try {
        ...
        for (const turn of result.turns || []) {
            ...
            for (const tool of turn.tools) {
                ...
                if (tool.name === "computer" && tool.input?.action === "left_click" && tool.input?.ref != null) {
                    steps.push({ tool: "computer", action: "left_click" });
                }
            }
        }
        if (steps.length >= 2) {
...
async function saveRestartState(checkpointPath, task, url, completedPages) {
    try {
        const dir = join(checkpointPath, "..");
        await mkdir(dir, { recursive: true });
        await writeFile(checkpointPath, JSON.stringify({ task, url, completedPages, savedAt: new Date().toISOString() }, null, 2), "utf8");
    }
    catch {
        // best-effort
    }
...
async function loadRestartState(checkpointPath, task) {
    try {
...
Options:
  ...
  --learning ./.local-agent/learning.json`);
    ...
    if (!target.webSocketDebuggerUrl)
        throw new Error("Selected Chrome target has no debugger URL.");
